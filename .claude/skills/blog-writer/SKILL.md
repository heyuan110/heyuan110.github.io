...
**内容深度**（⚠️ 这是最重要的质量标准，宁可少写一篇也不能水）：
- ✅ **必须有自己的观点和判断**——不能只陈述事实，要有"我认为"、"实际使用下来"、"这意味着"
- ✅ **每个论点必须有真实数据或例子支撑**——benchmark 数字、代码示例、实际使用场景
- ✅ **解释"为什么"而不只是"是什么"**——技术原理要讲到读者能举一反三
- ✅ **对比类必须有场景化推荐**——"什么场景用 A，什么场景用 B"，不能只列表格
- ✅ 复杂概念必须用类比或比喻解释
- ✅ 代码示例完整可运行，有英文注释
- ✅ **如果一个段落删掉也不影响核心论点，就该删掉**——宁可 3000 字深度文，不写 5000 字水文

**深度不够的典型症状（必须避免）**：
- ❌ 罗列功能清单但不解释为什么重要
- ❌ 引用官方文档但没有自己的判断
- ❌ 只说"X 很好"但不说好在哪里、和 Y 比有什么优势
- ❌ 架构讲了"是什么"但没讲"为什么这样设计"
- ❌ 全文没有一句"我认为"或"实际使用中发现"
- ❌ 一看就是 AI 生成的——结构太整齐、观点太中立、没有个人经验
