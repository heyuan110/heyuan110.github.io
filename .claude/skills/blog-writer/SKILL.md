---
name: blog-writer
description: "技术博客深度写作。不是内容生成器，是观点表达工具。写出有判断、有洞察、有经验感的技术文章。使用此 skill 当用户需要：(1) 撰写技术博客文章 (2) 整理技术资料成文章 (3) 介绍新工具/新技术"
---
...
```
确认需求 → 深度研究 → 形成判断（⛔ 必须通过） → 封面图 → 撰写 → 质量门禁 → 发布 → 分发
```
...
**⛔ 如果上面 6 个问题任何一个回答是空的或模糊的，禁止进入下一步。回去补充研究。**

---

### 步骤 4：封面图 + 配图

调用 `blog-cover-image` skill 生成封面图：
```
/blog-cover-image <文章目录> --quick
```

长文章可选调用 `blog-illustrator` 或 `blog-diagram` 生成配图。
图片统一英文生成，中英文版共用。
