...
[[params.faqItems]]
question = "子网掩码 255.255.255.224 对应的 CIDR 是多少？"
answer = "是 /27，可用主机数 30 个。换算方法就是数掩码里 1 的位数：每个 255 贡献 8 位，224 的二进制是 11100000 贡献 3 位，8+8+8+3=27。同样的算法放到别的位置也成立，255.255.224.0 就是 8+8+3=/19。常见邻居：/28（255.255.255.240）14 台，/26（255.255.255.192）62 台。"

[[params.faqItems]]
question = "192.168.1.0/24 里的 /24 到底是什么意思？"
answer = "CIDR 写法是「IP 地址/前缀长度」，前缀长度就是从左往右属于网络位的比特数。/24 表示前 24 位是网络 ID、后 8 位给主机，等价于子网掩码 255.255.255.0。很多工具要求填 CIDR 格式，意思就是写成 `192.168.1.100/24`，而不是地址加一个单独的掩码字段。CIDR 由 1993 年的 RFC 1518 和 RFC 1519 提出，用来取代浪费严重的 A/B/C 分类。"

[[params.faqItems]]
question = "一个子网能用多少个 IP？为什么要减 2？"
answer = "公式是「可用主机数 = 2^主机位数 - 2」。减掉的两个地址每个子网都必须保留：主机位全 0 的网络地址（如 192.168.1.0）和主机位全 1 的广播地址（如 192.168.1.255）。所以 /24 有 8 个主机位、254 个可用地址，/26 是 62 个，/27 是 30 个，点对点链路常用的 /30 正好剩 2 个。"

[[params.faqItems]]
question = "怎么把 192.168.1.0/24 划分成 4 个子网？"
answer = "借 2 个主机位（2^2=4），/24 变成四个 /26，掩码 255.255.255.192，每段 62 台可用。四段分别是 192.168.1.0/26（可用 .1-.62，广播 .63）、192.168.1.64/26（.65-.126，广播 .127）、192.168.1.128/26（.129-.190，广播 .191）、192.168.1.192/26（.193-.254，广播 .255）。通用步骤：先定子网数或每段主机数，算要借几位，推出新掩码，再列出各段范围。"

[[params.faqItems]]
question = "哪些 IP 段是私有地址？公网上不能路由的是哪几段？"
answer = "RFC 1918 定义了三段：10.0.0.0/8（10.0.0.0-10.255.255.255，约 1677 万个）、172.16.0.0/12（172.16.0.0-172.31.255.255，约 104 万个）、192.168.0.0/16（192.168.0.0-192.168.255.255，65536 个）。家用路由器和企业内网都用这几段，通过 NAT 访问公网。另外整个 127.x.x.x 段留给回环，255.255.255.255 是受限广播地址。"

[[params.faqItems]]
question = "Linux 下怎么查看 IP 配置、怎么算网络地址？"
answer = "`ip addr show` 列出所有网卡（加网卡名如 `ip addr show eth0` 只看一个），`ip route show` 看路由表。不想手算二进制与运算就用 `ipcalc 192.168.1.100/24`，它会一次性输出网络地址 192.168.1.0/24、子网掩码、广播地址 192.168.1.255、可用地址起止和主机数。连通性排查用 `ping 192.168.1.1` 和 `traceroute 8.8.8.8`。"
+++
