...
[[params.faqItems]]
question = "同样用 AI，为什么别人的产出比我好？"
answer = "差别不在模型，在工作流。多数人停在「问一句答一句」，然后抱怨 AI 给的东西太泛。真正要建立的是一套可复用的规则：哪个环节用哪个工具、提问前先给问题分类、提示词怎么迭代、代码怎么从 PRD 走到测试。一句话概括——你思路清晰，AI 是加速器；你思路混乱，AI 只是让你更快地量产垃圾。"

[[params.faqItems]]
question = "怎么判断一个问题该怎么问 AI？"
answer = "用四象限提问法。共识区（你很清楚要什么）：直接下指令，别啰嗦。盲区（你不懂的领域）：先让它给背景、适用场景和优劣对比，别急着要答案。私密区（涉及你自己的业务数据）：先喂足上下文，比如「跨境电商、东南亚市场、5000+ SKU、退货率 15%」，再让它分析。未知区（你自己也没想清楚要什么）：让它用苏格拉底式提问，一次只问一个问题。"

[[params.faqItems]]
question = "有没有快速提升 AI 回答质量的技巧？"
answer = "在提示词里指定一个思维框架。拆目标用 SMART，找根因用第一性原理，全面分析用 5W2H，简化方案用奥卡姆剃刀，抓关键因素用 80/20 法则。框架相当于 AI 的操作系统：没有它输出是发散的，有了它思考就有方向和结构。比如「用 SMART 框架拆解：3 个月内把博客月流量从 1000 做到 10000」。"

[[params.faqItems]]
question = "提示词到底怎么写？是不是一次就能写好？"
answer = "不可能一次到位，要走三步迭代。第一步先做调研，比如要写儿童绘本的提示词，先让 AI 研究「什么是好绘本」——故事结构、语言风格、画面节奏、年龄适配。第二步把调研结果喂回去，让它生成一版结构化的元提示词。第三步跑一遍，把结果贴回去并明确指出问题（人物表情僵硬、背景色太暗），让它针对这两点优化。重复 3-5 轮，提示词就相当精准了。"

[[params.faqItems]]
question = "想让 AI 生成特定风格的图片该怎么做？"
answer = "用对标复刻法：找一张目标风格的参考图发给 AI，让它输出 JSON 格式的元提示词（限制在 800 字以内），再把主体内容换成你自己的，大约能复刻 90% 的风格。如果手上没有参考图，别硬凑提示词，反过来让 AI 先问你四个问题：想达成什么效果、目标受众是谁、有没有参考案例、偏好什么色调和风格。"

[[params.faqItems]]
question = "AI 写代码怎么才能用在真实项目里？"
answer = "走规范驱动开发五步：PRD → 原型 → 调优 → 测试 → 后端。写 PRD 时别接受第一版，反复逼问边界情况、断网怎么办、高并发扛不扛得住。原型阶段用 AI IDE 的 Build 模式先跑通核心交互。然后把代码搬进正式 IDE 调优，AI 生成的代码通常「能跑但不好」——命名不规范、错误处理缺失、架构不清晰。测试环节让 AI 写用例并用 chrome-dev-tool MCP 跑自动化。后端最吃上下文，先让 AI 通读项目文档、导入 API 文档和数据库结构再开工。"

[[params.faqItems]]
question = "企业想落地 AI，从哪些场景切入风险最小？"
answer = "从已经验证过的低风险场景开始，别想一步到位全面智能化。问答类：把公司文档、制度、FAQ 导进去做内部知识库问答，对外的客服机器人也比传统规则机器人强得多。审核类：合同逐条审查标出风险项，流程合规自动校验。写作类：PRD、调研报告、季度复盘。查询类：Text-to-SQL，说一句「看下 Q1 销售额」就自动查库并可视化。开源平台可以试试 BISHENG，Apache 2.0 协议，商用免费。"
+++
