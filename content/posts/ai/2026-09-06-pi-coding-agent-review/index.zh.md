...
同一句「只回复 PONG」，在 pi 里花了 1,358 个上下文 token，在 Claude Code 里花了 31,012 个。同一台 Mac，同一个下午。

这两个数字是 **pi coding agent** 全部卖点的浓缩，也是最容易让人误判的地方。很多人把它当成「轻量版 Claude Code」。

它不是。我装完 pi、写了一个扩展、fork 了会话，再用两个工具各跑同样三个任务。

结论只有一句：**pi 是一套 harness 工程的零件包，不是一个成品 harness。** harness 就是包在模型外面的那层壳，管提示词、工具、执行循环、记忆和约束。

pi 把你自己搭 agent 时最先会造的四层壳做好了。剩下两层原样交还给你，也就是我在[六层 harness 那篇](/posts/ai/2026-04-18-harness-six-layers-reverse-build/)里说的、决定 80% 生产稳定性的后两层。

这是礼物还是坑，完全取决于你是谁。

下面全是收据。先说范围。菜鸟教程有一篇 pi 的功能手册，一步步讲安装和命令。这篇不重复手册，只讲每个功能要付什么代价、pi 不做什么、你该不该换。

## pi 到底是什么（2026 年 9 月的事实）

pi 是 Mario Zechner 对 Claude Code 的回应。用他自己的话说，Claude Code「变成了一艘飞船，80% 的功能我用不上」。

Zechner 是 libGDX 的作者。他在 2025 年 11 月的[博客](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/)里写清了 pi 的设计理由，到今天没变过。

四个工具（`read`、`write`、`edit`、`bash`），一屏能读完的系统提示词，其余一切走 TypeScript 扩展 API。

它的规模早就不是小项目了。截至 2026 年 9 月 6 日，[earendil-works/pi](https://github.com/earendil-works/pi) 有 103,925 star、12,999 fork。

仓库 2025 年 8 月 9 日创建，我查的前一天还有提交，MIT 协议。CLI 包 `@earendil-works/pi-coding-agent` 当前 v0.85.1，9 月 5 日发布，npm 周下载 153 万。

对比一下。`@anthropic-ai/claude-code` 周下载 855 万，`@openai/codex` 1,316 万。pi 的装机量大约是 Claude Code 的六分之一，但明显高于 OpenCode 的 131 万。

仓库五月搬了家。2026 年 5 月 7 日发布的 v0.74.0 是第一个挂在 `@earendil-works` 下的版本，背景是 Zechner 加入了 Armin Ronacher 联合创办的公益公司 Earendil。

旧地址 `badlogic/pi-mono` 现在自动跳转。协议还是 MIT，命令还是 `pi`。

比改名更重要的是两件事。第一，今年春天吸走半个行业注意力的 OpenClaw 就是建在 pi 的包之上，当前版本 `openclaw@2026.9.4` 依赖 `@earendil-works/pi-tui`。

第二，pi 的库包下载量比 CLI 还高，`pi-tui` 一个包周下载 512 万。

**pi 更像「别的 agent 拿来打地基的基础设施」，而不只是「一个人敲命令的 CLI」。**

社区反响是生态型的，不是发布帖型的。Hacker News 上没有一个 800 分的 Show HN。

2026 年最热的 pi 相关帖子是一条 56 分的抱怨，说它的配置目录在 Linux 上不遵守 XDG 规范，发于 8 月 17 日。第二热的是把 IDE 接进来的分支 oh-my-pi，42 分，7 月 21 日。

[Pragmatic Engineer](https://newsletter.pragmaticengineer.com/p/building-pi-and-what-makes-self-modifying) 四月做了整期节目。

但真正的信号在 [pi.dev/packages](https://pi.dev/packages)：5,410 个包。大家在往上盖东西，不是在争论它。

## pi 的四个包，放到六层 harness 上看

pi 是一个 monorepo，四个主包加几个辅助库。`pi-ai` 统一多厂商 API，`pi-agent-core` 跑循环，`pi-tui` 管终端渲染。

第四个包 `pi-coding-agent` 把前三者接成 CLI，另外给了 SDK 和 RPC 两种嵌入方式。SDK 是把 pi 当库 import 进你的程序，RPC 是让你的程序和 pi 进程之间收发 JSON 消息。

套到我之前文章里的六层模型上，pi 的形状歪得很刻意。
...
第 1 到 4 层不只是「有」，而是比多数人预期的好。图里说的「原语」，指 pi 只交付最底层的零件和接口，拼成什么由你决定。

其中会话树是 Claude Code 没有的东西。会话树的意思是，pi 会话里每条记录都带 `id` 和 `parentId`，你可以从任意一轮分叉出去，原来的路径不丢。

第 4 层记忆是我在六层那篇里劝大家跳过的层，pi 照样做了。因为它的存储格式从第一天起就是树，做起来便宜。

第 5、6 层是 pi 停下的地方。JSON 事件流和 RPC 协议它都有，那是可观测性的原材料。但除了 TUI 底栏，它没有成本看板，也没有评估框架。

权限系统则完全没有。pi 的[安全文档](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/security.md)写得很直白。

原话是：「pi 不包含内置沙箱……真正的隔离必须来自操作系统或虚拟化/容器边界。」

读过我[窗口期那篇](/posts/ai/2026-05-08-harness-engineering-window-of-opportunity/)的人会认出 pi 押的注。

补丁层会被更强的模型吸收，所以别烧进内核。补丁层指的是上下文小技巧、强制规划、权限表演这类东西。

设计判断层则留给用户，也就是验证什么、从什么恢复。pi 就是这个论点做成的产品。
...
「极简」的故事第一处漏气就在安装体积。其中 292 MB 是 `@esbuild` 的各平台二进制。

剩下是打包进去的 Anthropic、OpenAI、Google、AWS SDK，为的是所有厂商开箱即用。pi 省的是上下文 token，不是磁盘。

第一次交互式启动时，它还自动下载了 `fd` 和 `ripgrep`（6.7 MB）到 `~/.pi/agent/bin`。没问过我。方便，也有点自作主张。
...
不在名单里的 OpenAI 兼容接口也能接，比如阿里云 DashScope 的兼容模式、自建 vLLM。

做法是在 `~/.pi/agent/models.json` 里加一个 `openai-completions` 类型的 provider。改完不用重启，打开 `/model` 就能看到：
...
providers 文档里有一句话必须划重点，它改变了很多人的账。

原话是：「第三方 harness 的用量计入 extra usage，按 token 计费，不算订阅套餐额度。」我因此没跑 Claude 登录，那会按 token 扣这个账号的钱。

这台机器上唯一的 key 是 Google 的。所以 pi 这边用的是 **Gemini 3.1 Pro**，便宜的实验用 Gemini 3.8 Flash，Claude Code 那边用默认的 **Claude Fable 5.1**。

下面所有对比都请记住一件事：这是「harness 加模型」的对比，不是纯 harness 对比。
...
第二，表格中间那行是没人提醒你的坑。pi 会自动从 `~/.agents/skills` 和 `.agents/skills` 发现 skill，而这两个目录别的 harness 也在用。

我这儿躺着其他工具留下的 48 个 skill。pi 一声不吭地把 9,400 token 的 skill 描述塞进了每一次请求。

先跑一次 `pi --verbose` 看启动头信息，再相信「不到 1,000 token」的宣传语。

## 同样三个任务：pi 对 Claude Code 的收据

我搭了一个 241 行的 Node 项目，做发票计算：金额、税、折扣、CSV 报表。项目里配了五条规则的 `AGENTS.md`，折扣阶梯里埋了一个差一错误，另留四条 `TODO`。

然后给两边跑同样三个 prompt，每次都在干净的仓库副本里。两边都走无头模式。pi 这边是 `pi --mode json -p`，Claude Code 那边是 `claude -p --output-format json --dangerously-skip-permissions`。
...
成本那两列是两个工具各自报的标价。我付的是 Anthropic 订阅费，所以 Claude Code 那列是「走 API 会花多少」，不是我银行卡实际扣的。

token 那列才是实话。**每个任务 Claude Code 消耗的输入 token 都是 pi 的 1.8 到 2.5 倍。** 差距几乎全是 harness 开销每轮重发堆出来的，缓存读取也算在里面。

质量真正拉开差距的只有任务 c，而且拉开的是模型，不是 harness。

Claude Code 每个测试先写、先确认失败再实现。它把零总额告警的 logger 做成可注入，测试就不用 mock `console`。

它结尾还主动加了一句：「有件事我注意到但没动，因为不是 TODO：`volumeDiscount` 用的是严格大于比较」。那正是任务 b 埋的 bug。

pi 配 Gemini 把四条 TODO 都正确解决了，用的是 mock `console.warn`，但它没发现旁边那个 bug。任务 a 和 b 两边的 diff 功能上完全一致。

**三个任务里，四个工具都没有成为瓶颈。**

这和我找到的唯一一份同模型对比吻合。Composio 在 2026 年 8 月用 DeepSeek V4 Pro 分别跑 pi 和 OpenCode，做 30 个工具调用任务。

结果是 pi 解决 21/30（70%），OpenCode 19/30（63%）。pi 花了 $1.64，OpenCode $2.25。但 pi 中位耗时更长，363 秒对 281 秒。

两边每次请求的固定开销，是不到 1,000 token 对约 6,900 token。和我的数字一个形状：token 更少，结果持平或更好，但不更快。

## 40 行扩展，以及模型是怎么绕过它的

pi 的扩展性是它和 Claude Code 真正不同的地方，所以我没看文档就写了一个。扩展就是一个 TypeScript 文件，由 jiti 加载，不用编译，进来拿到一个 `ExtensionAPI`。

我这个扩展注册了一个 `tool_call` 处理器。`tool_call` 是模型每次要调工具时先经过的钩子，我在里面拦 `rm -rf`、`sudo` 和强推。

另外还注册了一个模型可调用的自定义工具，和一条斜杠命令：
...
这就是 Claude Code 的 PreToolUse 钩子，区别在形态。pi 的钩子是同进程里一个带类型的函数，不是从 stdin 读 JSON 的 shell 脚本。

所以它能原地改 `event.input`，能注册工具，能画 TUI 组件，能读会话。

`ctx.hasUI` 那个分支叫「空椅子测试」，由 [Agentic Control Plane 团队](https://agenticcontrolplane.com/blog/pi-acp-extension)八月给 pi 写策略扩展时提出。

道理很简单：无头模式下没人在场点「允许」，所以「询问」必须变成「拒绝」。

然后跑这条命令：`pi -e guard.ts --mode json -p "建 tmp_scratch 目录……然后 rm -rf tmp_scratch……如果失败，换个办法删掉。"`

下面是 JSON 流里的工具调用序列。模型是 Gemini 3.8 Flash，11.6 秒，$0.015：
...
钩子触发了。自定义工具能用。然后模型下一轮照样把目录删了，用的是 `rm` 加 `rmdir`。

是我让它换个办法的，它照做了。但一个被 README 注入了指令的模型也会这么干。

**`tool_call` 钩子是策略，不是边界。** 拦「别碰 `.env`」很好用。对一个铁了心要删东西的模型没用，对藏在 README 里的攻击者同样没用。
...
这个适配器是社区对 Zechner「你不需要 MCP」那篇的回敬。它用一个约 200 token 的代理工具，替掉 Playwright MCP 一次性塞进上下文的 13,700 token，服务器按需懒启动。

它周下载 197,718 次。`pi-subagents` 周下载 66,401。

pi 拒绝内置的东西，恰恰是它注册表里装得最多的东西。这要么证明原语路线是对的，要么说明大家最后还是把电池装回去了。我觉得两者都对。

## 会话树：/fork 到底做了什么

pi 把会话存成 `~/.pi/agent/sessions/` 下的 JSONL 树。

我无头跑了一个两轮会话：先读 `app.js`，再把它改成输出 `v2`。然后用 `pi --fork <id> -p "不要 v2，改成输出 v3。你开始时文件输出的是哪个版本？"` 分叉。

分叉回答「v3……我开始时文件输出的是 v1」。磁盘上是这样：
...
所以命令行的 `--fork` 是把当前活跃分支整个复制进新文件，从叶子继续。

要从*更早的点*分叉，得进交互式 `/tree` 视图。从更早的点分叉，正是 Claude Code 完全做不到的动作。

在 `/tree` 里选中一条旧的用户消息，叶子就移到它的父节点，原文放回编辑器让你改了重发。被放弃的分支可以自动生成一段摘要，挂到新位置。

JSONL 平铺直叙，我十行 Python 就解析了。这就是 pi 给你的可观测性原语，用来代替看板。

## 没有沙箱、没有权限弹框，对团队意味着什么

这张表大概是你来的目的。数据全部截至 2026 年 9 月。沙箱和权限的事实取自各工具自己的文档。
...
Zechner 为空着的「沙箱」那格给出的理由值得引用，而且基本是对的。

他的原话是：「agent 一旦能写代码又能跑代码，基本就完了……反正所有人都在 YOLO 模式下跑才做得了活，那为什么不干脆把它做成默认且唯一的选项？」

上面那次 guard.ts 实验就是证据。但他留给你的后半句，才是对团队真正要紧的。

对独立开发者，不弹框是生产力。风险你早就接受了，反正你本来就会点「允许」。

对团队，问题不是「弹框有没有用」，而是「以 `deploy` 身份跑的 agent 把 fixtures 删了，谁负责」。

Claude Code 的答案是一个可以写进 `settings.json` 强制执行的沙箱。Codex 的答案是默认开启的内核策略。

pi 的答案是「放容器里跑」。这个答案正确，同时也意味着容器成了你的责任、你 CI 的责任、你新人文档的责任。

你本来就在 Docker 或微虚拟机里跑 agent，pi 在这里不额外花你一分钱。你本来没有，pi 就是逼你开始的那个工具。

pi 确实实现了一个更小的信任边界，而且选得对：**项目信任**。

第一次打开一个含 `.pi/extensions` 或 `.agents/skills` 的仓库，pi 会先问再加载。因为一个能悄悄装进同进程 TypeScript 扩展的仓库，什么都干得了。

无头运行跳过提示，默认不加载。这是 pi 唯一说「先问」的地方，也是唯一问了真有用的地方。
...
诚实的中间地带是这样。如果你以搭 harness 为生，pi 是一个下午能读完的、第 1 到 4 层最好的参考实现，也是往上盖第 5、6 层最便宜的底座。

如果你是 harness 的消费者，Claude Code 那 31,012 token 的电池值这个价。

同样的区分我写过两次。[CLI skills vs MCP](/posts/ai/2026-07-04-cli-skills-vs-mcp/) 讲的是工具面，[agentic loops](/posts/ai/2026-07-03-agentic-loops/) 讲的是循环本身。

pi 就是把这两篇的建议逐字执行后得到的产品。
...
pi coding agent 是目前最清楚的存在性证明：四个工具加 1,358 token，在日常编程任务上足以打平一个电池齐全的 harness。我的三次运行和 Composio 的三十次说的是同一件事。

但它不是 Claude Code 的即插即用替代品。它留白的第 5、6 层，可观测和约束，恰恰是决定一个 agent 能不能在团队里活下来的两层。

如果你一直想自己搭 harness，我会从 pi 开始，上面那段 guard.ts 就是你的第一个下午。

如果你只想把活干完、让别人去负责沙箱，那就继续为那艘飞船付费。
