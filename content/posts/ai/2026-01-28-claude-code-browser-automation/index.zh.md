...
- [ego lite 实测：把你登录好的浏览器交给 Claude Code，token 省了多少](/zh/posts/ai/2026-09-21-ego-lite-browser-review/) — 本文对比的 2026 年 9 月更新：同样的任务在 ego lite、agent-browser、Chrome DevTools MCP 上实测，带真实 token 和费用收据

## 系列文章导航

这是「AI Agent 浏览器自动化」系列的第 2 篇。这条线的演进：headless 驱动 → 接管真实浏览器 → 把 token 砍到底 → 共享你的登录态：

1. [Vercel Agent Browser](/zh/posts/ai/2026-01-13-vercel-agent-browser/) — 为 Agent 而不是测试套件设计的 snapshot 式 CLI
2. **本文**：Claude Code 浏览器自动化：5 套方案实测对比 — 这条线的地图：token 成本、速度、稳定性
