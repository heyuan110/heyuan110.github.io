...
answer = "Spawn one when three conditions hold: the task reads far more context than it produces (roughly 10K+ input tokens), the result summarizes down to under 2K tokens, and the parent never needs the intermediate steps. Classic cases are codebase search, doc triage, log analysis, and parallel file inspection — a sub-agent can chew through a 500K-token log file and hand back one stack trace. Do not spawn one for multi-step reasoning that depends on shared memory: every sub-agent starts cold and cannot inherit the parent's working hypotheses."
[[params.faqItems]]
...
answer = "Route by decision complexity, not input volume. Opus for generation that needs judgment — refactors, architecture calls, writing. Sonnet for structured extraction, search-and-summarize, and PR review, where accuracy matters but synthesis does not. Haiku for classification, filtering, and mechanical passes like adding JSDoc to 50 functions. The common mistake is making Opus the orchestrator: restructuring one pipeline from an Opus orchestrator to a Sonnet orchestrator with an Opus writer and Haiku searchers cut end-to-end token cost by roughly 60% and improved output."
[[params.faqItems]]
...
answer = "A tool is one deterministic function call. An MCP server bundles related tools behind a standard protocol. A skill is a loadable instruction package that runs inside the same agent and shares its context. A sub-agent is a separate LLM invocation with its own context window, its own system prompt, and no access to the parent's working memory — which is also why it pays a cold start, re-tokenizing `CLAUDE.md` and every tool schema on each spawn. The defining property is context isolation: whatever it reads never pollutes the parent."
+++
