...
answer = "不是一回事。RAG 在查询时动态检索：文档切块、向量化、存进向量库，命中最相似的若干块再注入提示词。上下文工程则在交互前就把信息策划好，靠 CLAUDE.md 和结构化 docs/ 目录控制。RAG 适合大规模动态知识库，上下文工程适合需要确定性、可版本管理的项目规则。"

[[params.faqItems]]
question = "只用向量数据库做 Agent 记忆够吗？"
answer = "不够，生产级方案都是混合检索。Claude-Mem 同时跑 ChromaDB 向量相似度和 SQLite FTS5 关键词搜索再做融合排序——因为向量嵌入抓不住「401 错误」「JWT token」这类精确字符串。它还用渐进式披露：先返回 50-100 token 的索引，需要时才展开 500-1000 token 的完整观察，比粗暴注入省约 10 倍 token。"

[[params.faqItems]]
question = "200K 上下文窗口为什么还是不够用？"
answer = "因为 token 消耗是二次方增长，每次新的工具调用都要带上全部历史上下文。单次工具调用（读文件、跑命令、写代码）耗 1,000-10,000 token，一个中等复杂度任务要 50 次以上，大约 50 次之后 200K 窗口就被吃光。Claude Sonnet 4 是 200K、GPT-4o 128K、Gemini 1.5 Pro 2M，曲线一样陡。"

[[params.faqItems]]
question = "给 Agent 塞的上下文越多越好吗？"
answer = "恰恰相反，有四种公认的失效模式。上下文投毒会让一条过时规则被不断强化；上下文干扰在超过 32K token 后出现，模型开始重复近期模式；上下文混淆在暴露 40 个以上工具时拖垮判断；最狠的是上下文冲突——微软和 Salesforce 的研究测到平均 39% 的性能下降。"

[[params.faqItems]]
question = "RAG 和上下文工程能一起用吗？怎么分层？"
answer = "能，而且推荐四层结构。第一层 CLAUDE.md 放静态规则，进版本库、每次必加载；第二层结构化文档（设计、计划、指南）按任务选择性加载；第三层 RAG 记忆存对话历史和调试发现，用语义检索取回；第四层是当前会话的实时上下文——打开的文件、测试结果、报错信息。"

[[params.faqItems]]
question = "预算有限只做一件事，先做哪个？"
answer = "先写 CLAUDE.md。一个维护良好的文件写清技术栈、编码规范、架构决策，再加一段明确的「禁止做什么」，能用 20% 的投入拿到 80% 的收益，且不需要任何基础设施。等真的需要跨会话回忆再上 RAG 插件，起步参数保守些：只注入最近 10 次会话、最多 50 条观察。"
+++
