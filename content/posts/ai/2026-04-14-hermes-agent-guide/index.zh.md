...
draft = false
title = 'Hermes Agent v0.9 评测 + 安装指南：Nous Researc'
description = 'Nous Research 开源 Hermes Agent v0.9.0 实战评测：4 月 13 日发布、GitHub 27000+ stars、$5 Hetzner VPS 起步、自改进 + 自写...'
toc = true
...
```mermaid
flowchart TB
    Q([用户：帮我部署这个项目]) --> R{查询路由}
    R -->|发生过什么| E[情景记忆<br/>SQLite + FTS5<br/>'上次部署遇到端口冲突']
    R -->|你是谁| S[语义记忆<br/>持久状态<br/>'阿里云 ECS + Nginx 反代']
    R -->|怎么做| P[程序性记忆<br/>Skill markdown<br/>'deployment-checklist.md']
    E --> M[合成最小上下文]
    S --> M
    P --> M
    M --> A([Agent 执行])
