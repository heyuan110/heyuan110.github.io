...
这篇就是那套东西。课程内容本身我已经写过五篇精读（从[课程全解析](/posts/ai/2026-02-24-stanford-cs146s-overview/)开始），这篇是架在五篇之上的操作层：一张逐讲判断总表、工具映射、两条按起点区分的自学路线。先把我的核心立场放在最前面：**不要按顺序重放这门课，要倒着学**——大约 60% 的长期价值集中在第 3、4、6 三讲里，而占总评 80% 的 Final Project（不是阅读材料）才是这门课真正的主体。
...
| 2 | Agent 解剖与 MCP | ★★★ 全课最该动手的一个作业 | 徒手写一个 MCP Server，不用框架不用脚手架 | Claude Code + MCP SDK | [第一篇 §Week 2](/posts/ai/2026-02-24-stanford-cs146s-overview/) |
| 3 | 上下文工程 | ★★★ 全课 ROI 最高 | 写一页设计文档喂给 agent，和裸提示词跑同一任务做 diff | Claude Code / Cursor rules | [第二篇：上下文工程](/posts/ai/2026-02-24-context-engineering-deep-dive/) |
| 4 | Agent 协作模式 | ★★★ 从"写代码"到"管 Agent"的分水岭 | 用带检查点的方式指挥 agent 完整交付一个真实功能 | Claude Code plan mode | [第三篇：Agent Manager](/posts/ai/2026-02-24-agent-manager-patterns/) |
| 5 | 现代终端 | ★☆☆ 终端老手可跳过 | 扫一眼 Warp vs Claude Code 的定位文档即可 | Warp（或直接跳过） | Warp University |
| 6 | 测试与安全 | ★★★ demo 和产品之间的那道闸门 | 对自己 vibe coding 出来的仓库跑安全扫描，数真假阳性 | Semgrep + agent 复审 | [第四篇：安全 Vibe Coding](/posts/ai/2026-02-24-secure-vibe-coding/) |
| 7 | 代码审查 | ★★☆ 和第 6 讲捆着学 | 对抗式审查一个 AI PR：找出 agent 自己没发现的 3 个问题 | GitHub PR + 第二个 agent 当评审 | [第四篇：安全 Vibe Coding](/posts/ai/2026-02-24-secure-vibe-coding/) |
| 8 | 一句话建应用 | ★★☆ 好玩，但课在"差距"上 | 一句话生成一个应用，然后列出所有不能上生产的理由 | v0 / Lovable，然后 Claude Code | [第五篇：原型到生产](/posts/ai/2026-02-24-prototype-to-production/) |
| 9 | 部署后运维 | ★★☆ 读就行，别搭（除非你做运维） | 用"agent 值班"的视角读 Google SRE 导论 | 纯阅读周 | [第五篇：原型到生产](/posts/ai/2026-02-24-prototype-to-production/) |
| 10 | 软件工程的未来 | ★☆☆ 播客级内容，通勤听 | 有 Casado 演讲就看，没有就跳过 | — | [第一篇 §Week 10](/posts/ai/2026-02-24-stanford-cs146s-overview/) |
...
我给自己布置过、也建议你照做的练习：从积压需求里挑一个真实功能，写一页设计文档（约束、非目标、相关文件、业务规则），然后让同一个 agent 把这个任务跑两遍——一遍带文档、一遍裸提示词，diff 两次输出。我那次实验里，裸提示词版本凭空发明了一个数据模型，和两层目录之外的既有模型直接冲突；带文档的版本没有。那个 diff 就是这门课的中心论点在一个下午之内的可视化。这一讲的完整拆解见[精读第二篇：上下文工程](/posts/ai/2026-02-24-context-engineering-deep-dive/)；如果想知道课程之后这套方法在 2026 年哪些站住了、哪些被证明是伪技巧，我在[《上下文工程 2026》](/posts/ai/2026-06-16-context-engineering-2026/)里更新过立场。

### 第 4 讲：你从工程师变成管理者的那一讲

第 4 讲问的是全行业都在绕圈的问题：给 agent 多大自主权、检查点设在哪。阅读材料基本是一次 Claude Code 生态巡礼（嘉宾就是 Claude Code 之父 Boris Cherney），但真正耐久的内容是那条自主度光谱：简单任务放手跑，中等任务省 80% 时间但要人工收尾，复杂任务必须分段审查。作业——全程指挥 agent 而不是亲手敲代码地交付一个完整项目——是全课第二重要的练习，而且正好接在你第 3 讲写的设计文档后面。完整分析见[精读第三篇：Agent Manager](/posts/ai/2026-02-24-agent-manager-patterns/)。
...
让这一切变得具体的，是我给自己做的练习：把安全扫描加第二个 agent（当对抗式评审）指向一个我几周前基本靠 vibe coding 写完的仓库。扫出 11 个 findings，3 个是真的，其中一个真漏洞——一处未校验的重定向——出现在我记得当时"审过"的代码里。任何阅读材料都没有"发现自己亲手批准过的漏洞"这件事教育效果好。两讲的深度展开见[精读第四篇：安全 Vibe Coding](/posts/ai/2026-02-24-secure-vibe-coding/)。

### 第 8-10 讲：全景图，用播客速度过

第 8 讲（一句话生成应用）最好玩也最容易学歪：课的重点不是"你能生成一个应用"，而是那个应用和能收钱的产品之间的逐项差距。生成一个，然后写出"不能上生产的理由清单"——清单才是交付物。第 9 讲（部署后运维）除非运维是你的本职，否则当阅读周处理就好，SRE 遇上 agent 那套东西是行业去向的预告片，不是你下个迭代就会用的技能。第 10 讲是趋势座谈，配得上你的通勤时间，配不上你的书桌时间。三讲的内容都收在[精读第五篇：原型到生产](/posts/ai/2026-02-24-prototype-to-production/)里。
...
如果分流问题的答案是"还没有"，那就先别碰 CS146S——这门课默认你有 CS111 级别的编程底子加基本的工具经验，冷启动硬啃是收藏夹吃灰的标准路径。先花一周跟一份上手型入门（我的 [Vibe Coding 完全指南](/posts/ai/2026-02-22-vibe-coding-guide/)就是为这个写的），再走六周完整路线：第 1-2 讲带 MCP 作业、核心闭环、项目、模块四里挑一个选修。
...
- [斯坦福 CS146S 全解析（第一篇）：课程综述](/posts/ai/2026-02-24-stanford-cs146s-overview/) — 课程是什么、完整 10 周大纲
- [第二篇：上下文工程](/posts/ai/2026-02-24-context-engineering-deep-dive/) — 第 3 讲的深度展开
- [第三篇：Agent Manager](/posts/ai/2026-02-24-agent-manager-patterns/) — 第 4 讲的自主度光谱与协作模式
- [第四篇：安全 Vibe Coding](/posts/ai/2026-02-24-secure-vibe-coding/) — 第 6-7 讲的安全与审查
- [第五篇：原型到生产](/posts/ai/2026-02-24-prototype-to-production/) — 第 8-9 讲的交付与运维
- [上下文工程 2026：哪些方法真的有效](/posts/ai/2026-06-16-context-engineering-2026/) — 课程之后，这套方法的最新战况
- [Vibe Coding 完全指南 2026](/posts/ai/2026-02-22-vibe-coding-guide/) — 还没用 agent 交付过东西的人从这里进
