...
answer = "出现三个瓶颈之一就该拆：记忆膨胀（第一天 2 秒能响应的智能体，第三周可能要 8 秒）、上下文污染（跨领域知识互相干扰，回复跑题甚至幻觉出关联）、成本爆炸（每次请求都背着 50,000 token 的累积上下文，token 用量是必要量的 3-5 倍）。单智能体持续踩这些坑，就该按角色做专业化拆分。"

[[params.faqItems]]
question = "哪种多智能体模式最适合编码任务？"
answer = "编排者-工作者模式。编排者负责规划和拆任务，生成多个工作者并行实现与测试，最后合并结果并审查。Claude Code 的 Worktree 功能是教科书式实现：主实例为每个并行任务建一个独立的 `git worktree`，子智能体在隔离的文件系统分支里干活，互不冲突；Google Antigravity 的 Manager View 是同一模式的 GUI 版。"

[[params.faqItems]]
question = "多智能体系统中智能体之间如何通信？"
answer = "靠结构化消息传递，通常是内部 API 上跑的 JSON。原则是最小且结构化：只传下一步真正需要的字段，用 schema 约束而不是自由文本，避免误解层层放大。项目级的共享规范不走消息，写进 `CLAUDE.md` 由每个智能体会话继承，省得在每条提示里重复一遍。"
+++
