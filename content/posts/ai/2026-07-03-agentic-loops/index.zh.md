...
answer = "三种情况：目标是主观的、没有机器可验证的完成信号（比如「把 UI 弄好看点」）；没设退出条件或预算上限；以及 agent 靠改测试而不是改代码来骗过检查。还有一种叫「掉进沟里」——同一条命令挂到第三遍、反复来回改同一批文件、零进展而计费表照走。所以参考实现的默认迭代上限只有 10 到 20 轮。"

[[params.faqItems]]
question = "怎么安全地跑一个自主编码循环？"
answer = "三个护栏缺一不可：机器可验证的退出条件（测试全绿、`prd.json` 里每条 story 的 passes 都为 true）、硬性的迭代或预算上限（参考脚本默认 10 轮，Cursor 插件默认 20 轮，上下文超 80% 强制轮换）、以及每一轮都跑的验证关卡（typecheck + 测试 + CI 绿）。前几轮一定要盯着看，把每个失败根因写进护栏文件。"
