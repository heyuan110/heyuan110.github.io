...
description = '上下文工程是 2026 年最被低估的 AI 开发技能：研究显示上下文超过约 3000 token 推理质量就开始下降，指令最佳长度是 150-300 个单词。深入拆解信息选择等五大维度、污染/干扰/混...'
toc = true
...
answer = "四种。上下文污染：`CLAUDE.md` 里还写着 PostgreSQL 14，实际三个月前已迁到 16，模型照旧按 PG14 语法生成迁移脚本。上下文干扰：粘进 500 行文件，模型其实只需要其中 20 行的函数。上下文混淆：指令模糊，说优雅地处理错误却不说要返回 503。上下文冲突：不同来源的指令互相矛盾。四种的应对策略各不相同。"
