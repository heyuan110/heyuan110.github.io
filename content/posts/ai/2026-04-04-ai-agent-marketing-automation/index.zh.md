...
注意最后那句"每个视频开一个 issue"。这不是随便加的。这条指令确保了每一条内容都有据可查——后面的反馈循环全靠这个 issue 系统。没有记录就没有学习，没有学习就没有进化。

然后他创建了一个 Routine（定时任务）：每天自动检查哪些视频播放量高、哪些拉垮。DevRel 在 issue 里加评论——"这个开头不够抓人""BGM 选得不好"。AI 学完以后，下一批视频质量直接上一个台阶。
