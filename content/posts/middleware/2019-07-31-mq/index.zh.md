...
title = '消息队列 MQ 技术选型指南：核心概念与分布式系统应用场景'
description = '消息队列技术选型完整指南，解析 MQ 核心概念（Producer、Broker、Consumer）、架构模型，以及在分布式系统中的异步处理、削峰填谷、系统解耦等应用场景'
toc = true
...
[[params.faqItems]]
question = "消息队列到底是什么？"
answer = "是一种存放传输中消息的中间件，核心作用是可靠传输——接收方不在线时消息先存着，直到投递成功。它由 3 个角色组成：生产者负责生成并发送消息，Broker 负责存储、确认、重试并管理多个队列，消费者负责取出消息并处理。在分布式系统里主要用来做异步提速、削峰填谷和服务解耦。"

[[params.faqItems]]
question = "用消息队列能解决什么问题？"
answer = "3 件事。解耦：A 系统只管发消息，B、C 以及之后加入的 D 各自订阅，新增消费者不用改 A，下游挂了也不会连累上游。异步：同步模式耗时 120s 的流程，把非核心环节 B、C 移到异步后只要 35s。削峰：平时每秒 50 个请求、中午突增到 5k+，而 MySQL 最大只能扛每秒 2k，有队列就先写入再按 2k 的速度消费，高峰期短暂积压，过后很快消化完。"

[[params.faqItems]]
question = "引入消息队列有什么代价？"
answer = "2 个明显代价。一是降低系统可用性——Broker 成了新的故障点，你必须额外处理 MQ 宕机和消息丢失。二是增加系统复杂度——直接 API 调用只有一跳，用了队列就要处理消息投递保证、重复消费、跨服务数据一致性，链路变长延迟也更高。如果同步调用本来就够用，加队列只会让系统更糟。"

[[params.faqItems]]
question = "什么场景该用消息队列，什么场景不该用？"
answer = "3 类场景该用。一是数据驱动的任务依赖：订单同步、库存更新、生成拣货单这种串行任务，传统做法是定时任务加预留缓冲时间，改成每个任务完成后发一条完成消息、下游订阅后立即启动（队列只传信号不传数据）。二是上游不关心下游结果，比如下单后触发确认邮件。三是执行时间长需要异步返回结果，比如支付流程。"

[[params.faqItems]]
question = "RabbitMQ 和 Kafka 怎么选？"
answer = "中小公司优先 RabbitMQ——管理方便、社区活跃、文档齐全、多语言支持好，这些比吞吐量数字更重要。Kafka 更适合大数据管道和高吞吐流式场景。备选还有 RocketMQ 和 ActiveMQ。核心原则是技术选型必须由实际业务需求驱动：每秒 50 个请求偶尔冲到 5k 的系统，和每秒十万级事件的管道，需要的根本不是同一个东西。"
+++
