...
红线:
  ...
  4. 整组答案里至少半数(且≥2条)含具体信息:数字、价格、行内代码、文件名、路径或命令行参数。
     纯定义型问答可以没有(文件名和路径同样算具体信息,不要为了凑数字而硬塞)
...
HAS_FACT = re.compile(
    ...
    r"|`[^`]+`"                # 行内代码
    r"|\$"                     # 价格
    r"|[\w./-]+\.(?:md|json|toml|ya?ml|sh|py|ts|js|txt|conf|lock)\b"  # 文件名
    r"|(?:^|\s)[~.]?/[\w./-]{2,}"   # 路径
    r"|(?:^|\s)--?[a-zA-Z][\w-]{1,}" # 命令行参数
)
